...
import { useLocalStore, useSingletonStore } from '@/lib/dataStore'

export default function Services() {
  const { overrides } = useImageOverrides()
  const { items: SERVICES } = useLocalStore(serviceCategoriesStore)
  const { value: content } = useSingletonStore(homepageContentStore)

  return (
    <section id="services" className="scroll-mt-24 bg-[#faf9f6] pb-24 md:pb-32">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        <Reveal className="text-center">
          <p className="text-[12px] font-medium uppercase tracking-[0.3em] text-neutral-500">{content.services.eyebrow}</p>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {SERVICES.map((s, i) => (
            <Reveal key={s.slug} delay={i * 110}>
              <SmartLink href={`/services/${s.slug}`} className="group relative block overflow-hidden rounded-2xl">
                <div className="relative aspect-[3/4] w-full">
                  <img
                    src={resolveImage(overrides, `services.${s.slug}`, s.image)}
                    alt={s.title}
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-7 text-white">
                    ...
                    <p className="mt-3 max-w-xs text-[14px] leading-relaxed text-white/75">{s.cardText}</p>
